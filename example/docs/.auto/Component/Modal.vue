<template>
  <div class="component-modal">
    ...
    <h2
      id="modal-dui-hua-kuang"
      class="mz-heading mz-document-heading"
      data-level="2"
    >
      <a
        class="mz-document-anchor"
        href="#modal-dui-hua-kuang"
        title="Modal 对话框"
        data-level="2"
        data-href="#modal-dui-hua-kuang"
        >¶</a
      >Modal 对话框
    </h2>
    ...
    <demo-block>
      <template #example
        ><modal-demo1 inline-template
          ><div class="modal-demo1">
            <mz-button color="success" @click="visible = true"
              >显示对话框</mz-button
            >
            <mz-modal v-model="visible" title="对话框">
              <div>Welcome to chaos world!</div>
              <template #footer>
                <mz-button flat color="primary" @click="visible = false"
                  >呦吼</mz-button
                >
              </template>
            </mz-modal>

            <mz-button color="warning" @click="visible2 = true"
              >有分隔线的对话框</mz-button
            >
            <mz-modal v-model="visible2" divider title="分隔线效果对话框">
              <div>全てはシュタインズゲートの选択だ</div>
              <template #footer>
                <mz-button flat color="primary" @click="visible2 = false"
                  >是的</mz-button
                >
              </template>
            </mz-modal>

            <mz-button color="danger" @click="visible3 = true"
              >显示无头对话框</mz-button
            >
      ...
      <template #description
        ><p>
          <code>visible/v-model</code
          >属性控制对话框的显示隐藏。<code>divider</code>属性控制是否使用分割线分隔。不使用<code>title</code>属性且不用<code>title</code>插槽，将不渲染对话框头部。
        </p>
      </template>
      <template #highlight
        >><pre v-pre><code class="html">&lt;mz-button color=&quot;success&quot;
...
&lt;/script&gt;
    ...
    >
      <a
        class="mz-document-anchor"
        href="#nei-wai-gun-dong"
        title="内外滚动"
        data-level="3"
        data-href="#nei-wai-gun-dong"
        >¶</a
      >内外滚动
    </h3>
    <p>提供了两种滚动条效果。</p>
    <demo-block>
      <template #example
        ><modal-demo2 inline-template
          ><div class="modal-demo2">
            <mz-button color="success" @click="show(false)"
              >内滚动对话框</mz-button
            >
            <mz-button color="warning" @click="show(true)"
              >外滚动对话框</mz-button
            >
            <mz-modal
              v-model="visible"
      ...
      >
      <template #description
        ><p><code>outer-scroll</code>属性控制是否使用外部滚动条。</p>
      </template>
      <template #highlight
        >><pre v-pre><code class="html">&lt;mz-button color=&quot;success&quot;
...
&lt;/script&gt;
    ...
    >
    <h3
      id="qian-tao-de-dui-hua-kuang"
      class="mz-heading mz-document-heading"
      data-level="3"
    >
      <a
        class="mz-document-anchor"
        href="#qian-tao-de-dui-hua-kuang"
        title="嵌套的对话框"
        data-level="3"
        data-href="#qian-tao-de-dui-hua-kuang"
        >¶</a
      >嵌套的对话框
    </h3>
    <p>对话框内触发内部对话框显示。</p>
      ...
      <template #example
        ><modal-demo3 inline-template
          ><div class="modal-demo3">
            ...
            <mz-modal v-model="visible" title="对话框">
              <div>Welcome to chaos world!</div>
              <mz-modal v-model="visible2" append-to-body title="嵌套的对话框">
                <div>全てはシュタインズゲートの选択だ</div>
                <template #footer>
                  <mz-button flat color="primary" @click="visible2 = false"
                    >关闭</mz-button
              ...
              <template #footer>
                <mz-button flat color="primary" @click.stop="visible = false"
                  >关闭</mz-button
                >
                <mz-button flat color="success" @click="visible2 = true"
                  >显示对话框</mz-button
                >
              </template>
            </mz-modal>
      ...
      <template #description
        ><p>
          <span style="color:var(--color-danger)"
            >原则上禁止嵌套弹窗，因为这样做可能会导致弹窗和遮罩的层级的关系错误。</span
          >如果需要在页面上同时显示多个
          Dialog，可以将它们平级放置。如果万不得已一定要用，Model
          组件提供了<code>append-to-body</code>属性，将内层 Model 的该属性设置为
          true，它就会插入至 body 元素上，从而保证内外层 Model
          和遮罩层级关系的正确。
        </p>
      </template>
      <template #highlight
...
&lt;/script&gt;
    ...
    >
      <a
        class="mz-document-anchor"
        href="#guan-bi-lan-jie"
        title="关闭拦截"
        data-level="3"
        data-href="#guan-bi-lan-jie"
        >¶</a
      >关闭拦截
    </h3>
    <demo-block>
      <template #example
        ><modal-demo4 inline-template
          ><div class="modal-demo4">
            <mz-button color="success" @click="show">显示对话框</mz-button>
      ...
      <template #description
        ><p>
          <code>before-close</code>属性可以拦截关闭事件，传入 function(done)
          方法，<code>done</code>为关闭的方法。你也可以传入 Promise。
        </p>
      </template>
      <template #highlight
        >><pre v-pre><code class="html">&lt;mz-button color=&quot;success&quot;
...
&lt;/script&gt;
    ...
    >
    <h3
      id="quan-ping-dui-hua-kuang"
      class="mz-heading mz-document-heading"
      data-level="3"
    >
      <a
        class="mz-document-anchor"
        href="#quan-ping-dui-hua-kuang"
        title="全屏对话框"
        data-level="3"
        data-href="#quan-ping-dui-hua-kuang"
        >¶</a
      >全屏对话框
    </h3>
    <demo-block>
      <template #example
        ><modal-demo5 inline-template
          ><div class="modal-demo5">
            <mz-button color="success" @click="visible = true"
              >全屏对话框</mz-button
            >
      ...
      >
      <template #description
        ><p><code>fullscreen</code>属性控制对话框是否全屏显示。</p>
      </template>
      <template #highlight
        >><pre v-pre><code class="html">&lt;mz-button color=&quot;success&quot;
...
&lt;/script&gt;
    ...
    >
    <h3
      id="jin-yong-dian-ji-zhe-zhao-guan-bi"
      class="mz-heading mz-document-heading"
      data-level="3"
    >
      <a
        class="mz-document-anchor"
        href="#jin-yong-dian-ji-zhe-zhao-guan-bi"
        title="禁用点击遮罩关闭"
        data-level="3"
        data-href="#jin-yong-dian-ji-zhe-zhao-guan-bi"
        >¶</a
      >禁用点击遮罩关闭
    </h3>
    <demo-block>
      <template #example
        ><modal-demo6 inline-template
          ><div class="modal-demo6">
            <mz-button color="success" @click="visible = true"
              >显示对话框</mz-button
            >
            <mz-modal
      ...
      <template #description
        ><p>
          <code>close-on-click-mask</code>属性控制对话框是否允许点击遮罩关闭。
        </p>
      </template>
      <template #highlight
        >><pre v-pre><code class="html">&lt;mz-button color=&quot;success&quot;
...
&lt;/script&gt;
    ...
    >
    <h3
      id="geng-duo-dong-hua"
      class="mz-heading mz-document-heading"
      data-level="3"
    >
      <a
        class="mz-document-anchor"
        href="#geng-duo-dong-hua"
        title="更多动画"
        data-level="3"
        data-href="#geng-duo-dong-hua"
        >¶</a
      >更多动画
    </h3>
    <demo-block>
        ...
        ><modal-demo7 inline-template
          ><div class="modal-demo7">
            ...
            <mz-button color="success" @click="visible = true"
              >显示对话框</mz-button
            >
            <mz-modal v-model="visible" title="对话框" :transition="transition">
              <div>Welcome to chaos world!</div>
              <template #footer>
                <mz-button flat color="primary" @click="visible = false"
                  >关闭</mz-button
      ...
      <template #description
        ><p>
          ...
          >属性控制对话框的过渡动画。默认<code>mz-zoom</code>。
        </p>
      </template>
      <template #highlight
        >><pre
          v-pre
        ><code class="html">&lt;span class=&quot;demo-font-size-14&quot;&gt;动画：&lt;/span&gt;
...
&lt;/script&gt;
    ...
    >
    <h3
      id="kuai-jie-diao-yong"
      class="mz-heading mz-document-heading"
      data-level="3"
    >
      <a
        class="mz-document-anchor"
        href="#kuai-jie-diao-yong"
        title="快捷调用"
        data-level="3"
        data-href="#kuai-jie-diao-yong"
        >¶</a
      >快捷调用
    </h3>
    ...
    <demo-block>
      <template #example
        ><modal-demo8 inline-template
          ><div class="modal-demo8">
            <!-- alert -->
      ...
      <template #highlight
        >><pre v-pre><code class="html">&lt;!-- alert --&gt;
&lt;mz-button @click=&quot;alert&quot;&gt;Alert&lt;/mz-button&gt;
&lt;mz-button @click=&quot;confirm&quot;&gt;Confirm&lt;/mz-button&gt;
&lt;mz-button @click=&quot;prompt&quot;&gt;Prompt&lt;/mz-button&gt;
&lt;mz-button @click=&quot;promptWithRule&quot;&gt;带校验的Prompt&lt;/mz-button&gt;
...
&lt;/script&gt;
    ...
    <h4 id="modal" class="mz-heading mz-document-heading" data-level="4">
      <a
        class="mz-document-anchor"
        href="#modal"
        title="$modal"
    ...
    <div style="padding-left: 20px; font-size: 14px;">
      ...
      <ul class="mz-document-ul">
        ...
        <li>
          close-on-click-mask &lt;Boolean&gt; 是否可以通过点击遮罩关闭对话框
          (true)
        </li>
      ...
      methods: {
        ...
        confirm() {
          this.$modal.confirm({
            ...
            cancel: {
              callback: () => {
                this.close('Confirm 取消')
              }
            },
            confirm: {
              callback: () => {
                this.close('Confirm 确认')
              }
            }
          })
        },
